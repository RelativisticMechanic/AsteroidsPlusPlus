...
vec4 burnInEffect(in vec2 uv, in sampler2D prev_frame, in sampler2D curr_frame)
{
    ...
    highp float decay = clamp((1.0/(current_burn - last_burn)) * 120.0, 0.0, 1.0);
    old_color *= decay;
...
vec4 crtGlow(in sampler2D crt_texture, in vec2 uv, in float blurSize)
{
    ...
    vec4 texture_color = mix(burnInEffect(uv, burn_in_source, tex), vec4(accent_color, 1.0), 0.25);
    vec4 result = sum * (intensity + flicker_fraction * intensity * sin(time*flicker_speed)) + vec4(length(texture_color.rgb) * accent_color, texture_color.a);
...
void main(void)
{
    ...
    if ((uv.y > 1.0 || uv.x < 0.0 || uv.x > 1.0 || uv.y < 0.0) && warp > 0.0)
    {
        ...
        /* Add the moving scanline */
        fragColor.rgb += fract(smoothstep(-1.0, 0.0, uv.y - 1.0 * fract(time * 0.1976))) * scanline_intensity * accent_color;
